...
function ManageProducts() {
  ...
  const handleRemove = (productId, index) => {
    fetch(`${API}/product/${productId}/${user._id}/`, {
      method: "DELETE",
      headers: {
      ...
      .then((data) => {
        let list = data.map((product) => {
          return {
            ...
            updatedAt: new Date(product.updatedAt).toString(),
          };
        });
        setProducts(list);
      })
                ...
                <tbody>
                  {products.map((product, index) => (
                    <tr key={index} className="border-b hover:bg-green-50">
                      ...
                      <td className="px-3 py-4 uppercase">{product.name}</td>
                      <td className="px-3 py-4">{product.createdAt}</td>
                      <td className="px-3 py-4">{product.updatedAt}</td>
